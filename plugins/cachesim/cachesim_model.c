...
static inline int cachesim_get_set(cachesim_model_t *cache, addr_t addr) {
  int set = ((int)addr) >> cache->set_shift;
...
int cachesim_model_init(cachesim_model_t *cache, char *name, unsigned size,
  ...
  cache->sets = size / (line_size * assoc);
  cache->lines = calloc(cache->sets * assoc, sizeof(cachesim_model_line_t));
  if (cache->lines == NULL) {
    return -1;
  }
...
void cachesim_load_line(cachesim_model_t *cache, int line_index, addr_t addr, bool is_write) {
  ...
  cache->lines[line_index].tag = (addr >> cache->tag_shift) << 1;
}
  ...
  for (; addr < end; addr += cache->line_size) {
    ...
    if (!hit) {
      ...
      if (cache->parent) {
        ...
        assert(ret == 0);
        cachesim_ref(cache->parent, addr, size, is_write);
