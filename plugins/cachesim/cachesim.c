...
void cachesim_proc_buf(cachesim_thread_t *cachesim_thread) {
  cachesim_trace_t *cachesim_buf = &cachesim_thread->trace_buf;

  for (int i = 0; i < cachesim_buf->len; i++) {
    cachesim_ref(&cachesim_thread->l1d_model, cachesim_buf->entries[i].addr,
                 cachesim_buf->entries[i].info >> 1, cachesim_buf->entries[i].info & 1);
  }
  cachesim_buf->len = 0;
}
...
int cachesim_pre_inst_handler(mambo_context *ctx) {
  ...
  if (is_load || is_store) {
    ...
    emit_set_reg_ptr(ctx, 2, &cachesim_thread->trace_buf.entries);
...
int cachesim_pre_thread_handler(mambo_context *ctx) {
  ...
  int ret = cachesim_model_init(&cachesim_thread->l1d_model, "L1d", L1D_SIZE,
                                L1D_LINE_SIZE, L1D_ASSOC, L1D_REPL);
  assert(ret == 0);
  cachesim_thread->l1d_model.parent = &l2_model;

  cachesim_thread->trace_buf.len = 0;
...
int cachesim_post_thread_handler(mambo_context *ctx) {
  cachesim_thread_t *cachesim_thread = mambo_get_thread_plugin_data(ctx);
  cachesim_proc_buf(cachesim_thread);

  for (int i = 0; i < 2; i++) {
    atomic_increment_u64(&global_l1d.stats.references[i], cachesim_thread->l1d_model.stats.references[i]);
    atomic_increment_u64(&global_l1d.stats.misses[i], cachesim_thread->l1d_model.stats.misses[i]);
    atomic_increment_u64(&global_l1d.stats.writebacks[i], cachesim_thread->l1d_model.stats.writebacks[i]);
  }
...
__attribute__((constructor)) void cachesim_init_plugin() {
  ...
  // These models aren't used, they just store the global L1 stats and configuration
  int ret = cachesim_model_init(&global_l1d, "L1d", L1D_SIZE,
                                L1D_LINE_SIZE, L1D_ASSOC, L1D_REPL);
  assert(ret == 0);
  ret = cachesim_model_init(&global_l1i, "L1i", L1I_SIZE,
                            L1I_LINE_SIZE, L1I_ASSOC, L1I_REPL);
  assert(ret == 0);

  ret = cachesim_model_init(&l2_model, "L2", L2_SIZE,
                            L2_LINE_SIZE, L2_WAYS, L2_REPL);
  assert(ret == 0);
